...
/-- Applicative/monad coherence: `gseq` agrees with monadic sequencing
`f >>=ᵍ fun f' => f' <$>ᵍ x`. This holds *definitionally* because `gseq` for
`Parser` is defined via `bind`, but `LawfulGradedMonad` does not include a
`gseq = gbind ...` coherence law, so downstream code cannot rely on it without
this lemma. (Lean core's `LawfulMonad` requires the analogous `seq_eq_bind`.) -/
@[simp] theorem gseq_eq_gbind {i j : Grade} (f : Parser ε i (α → β)) (x : Parser ε j α) :
    (f <*>ᵍ fun () => x) = (f >>=ᵍ fun f' => f' <$>ᵍ x) := rfl
